...
const Stars = ({ rating }) => {
    ...
    return (
        <div className="flex items-center gap-0.5">
            {[...Array(full)].map((_, i) => (
                <svg key={`f${i}`} className="w-3.5 h-3.5 text-[#ed7844]" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
            ))}
            {half && (
                <svg className="w-3.5 h-3.5 text-[#ed7844]" fill="currentColor" viewBox="0 0 20 20">
            ...
            <span className="ml-1 text-xs font-bold text-gray-500">{(rating || 0).toFixed(1)}</span>
        </div>
    );
};
...
const ProductCard = ({ product }) => {
    ...
    return (
        <div
            ...
            className="group relative border-2 border-gray-200 bg-white overflow-hidden transition-all duration-300 hover:border-black hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-1 flex flex-col h-full cursor-pointer"
        >
            {/* Category badge */}
            {product.category && (
                <div className="absolute top-3 left-3 z-10 bg-black/80 text-white text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 backdrop-blur-sm">
                    {product.category}
                </div>
            )}
            ...
            <div className="p-4 flex flex-col flex-grow">
                ...
                <div className="mt-auto pt-3 border-t border-gray-100">
                    <div className="flex items-center justify-between mb-3">
                        <span className="text-xl font-black text-black">${product.price?.toFixed(2) || '0.00'}</span>
                    </div>
                    ...
                    <div className="relative flex gap-2">
                        <button
                            ...
                            className={`flex-1 px-3 py-2 text-xs font-bold uppercase tracking-wider border-2 transition-all duration-200 ${
                                addedToCart 
                        ...
                        <button
                            ...
                            className="px-2.5 py-2 text-xs font-bold border-2 border-gray-300 text-gray-500 hover:border-black hover:text-black transition-all duration-200"
